#!/usr/bin/env Rscript
# Test: Three tests with ALL loadings FREE, factor variance FIXED to 1.0
# This matches the Roy model's identification strategy
...
true_T1_lambda <- 1.0  # Will be FREE (estimated)
true_T1_sigma <- 0.5

true_T2_intercept <- 0.5
true_T2_lambda <- 1.2  # Will be FREE (estimated)
true_T2_sigma <- 0.6

true_T3_intercept <- -0.3
true_T3_lambda <- 0.8  # Will be FREE (estimated)
true_T3_sigma <- 0.4
...
cat("=== Three Tests: All Loadings FREE, Factor Var FIXED ===\n\n")
...
# Define model - ALL loadings FREE (no loading_normalization)
...
# Parameters: factor_var (FIXED), T1_int, T1_lambda, T1_sigma, T2_int, T2_lambda, T2_sigma, T3_int, T3_lambda, T3_sigma
...
delta <- 1e-7
param_names <- c("factor_var", "T1_int", "T1_lambda", "T1_sigma", "T2_int", "T2_lambda", "T2_sigma",
