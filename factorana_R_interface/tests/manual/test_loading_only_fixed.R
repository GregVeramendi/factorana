#!/usr/bin/env Rscript
# Test: Fix ONLY the loading (not factor variance) for identification
...
# True parameters
true_factor_var <- 1.0  # True factor variance
true_T1_intercept <- 1.0
true_T1_lambda <- 1.0  # Will be FIXED for identification
true_T1_sigma <- 0.5
...
cat("=== Three Test Scores - Fix Loading Only (Not Factor Variance) ===\n\n")
cat(sprintf("n = %d observations\n", n))
cat(sprintf("True parameters:\n"))
cat(sprintf("  Factor variance: %.2f (will be ESTIMATED, not fixed)\n", true_factor_var))
cat(sprintf("  T1: intercept=%.2f, lambda=%.2f (FIXED for ID), sigma=%.2f\n",
...
# Define model - fix T1 loading to 1.0, estimate factor variance
...
# Parameters: factor_var (FREE!), T1_int, T1_sigma, T2_int, T2_lambda, T2_sigma, T3_int, T3_lambda, T3_sigma
...
# Initialize parameters - ALL FREE (don't fix factor_var)
init_params <- c(
  1.0,  # factor_var (initial guess)
...
param_fixed <- rep(FALSE, 9)

cat("Starting parameter estimation...\n")
cat("(All parameters free - T1 loading fixed via normalization, not param fixing)\n\n")
...
opt_result <- nlminb(
  ...
  lower = c(0.01, rep(-Inf, 8)),  # Factor variance must be positive
