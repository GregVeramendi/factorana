#!/usr/bin/env Rscript
# Test binary probit model implementation with parameter recovery
# Demonstrates: binary outcome with no latent factors

library(factorana)

cat("========================================\n")
cat("Binary Probit Model Test\n")
cat("========================================\n\n")
...
# Generate binary outcome via probit
latent <- true_beta0*intercept + true_beta_x1*x1 + rnorm(n)
D <- as.integer(latent > 0)

dat <- data.frame(D = D, intercept = intercept, x1 = x1, eval = 1)

cat("  Sample size:", n, "\n")
cat("  P(D=1):", round(mean(D), 3), "\n")
cat("  Distribution:\n")
print(table(D))
cat("\n")

# Define probit model (no factors)
...
# Parameters: [sigma_f^2, beta0, beta_x1]
test_params <- c(1.0, 0.5, 0.8)
...
cat("  Gradient values:\n")
param_names <- c("sigma_f^2", "beta0", "beta_x1")
...
cat("\n")
if (all_passed) {
  cat("  ✓✓✓ All probit model gradients match within tolerance!\n\n")
} else {
...
# Generate binary outcome via probit
latent_sim <- true_beta0_sim*intercept_sim + true_beta_x1_sim*x1_sim + rnorm(n_sim)
D_sim <- as.integer(latent_sim > 0)
...
dat_sim <- data.frame(D = D_sim, intercept = intercept_sim,
                      x1 = x1_sim, eval = 1)
...
# Parameter order: [sigma_f^2 (fixed), beta0, beta_x1]
cat("Parameter Recovery:\n")
cat(sprintf("  %-12s %12s %12s %12s %10s\n",
            "Parameter", "True", "Estimated", "Difference", "Status"))
cat(strrep("-", 60), "\n")

true_vals <- c(true_beta0_sim, true_beta_x1_sim)
est_vals <- c(estimates[2:3])
param_labels <- c("beta_0", "beta_x1")
...
cat("========================================\n")
cat("Binary Probit Model Test Complete!\n")
cat("========================================\n\n")
